...
Ethically, it is important to make a conscious effort to put our analyses in broader social, economic, and historical context when considering inequalities. Intersectionality plays a big role in environmental justice, so factors such as race, geographic location, and age must be considered when drawing conclusions in this exploratory data analysis.
...
There are a large number of missing values for some variables. To investigate this issue further, the number of missing values for each state across all features were computed. Grouping by state allows us to see if certain geoareas have less coverage from EJ screen.
...
This table shows summary statistics for all numerical variables in the EJScreen dataset. Even after filtering out 100 variables, this table is still too large to draw meaningful insights. Collectively, our team looked at the variables and isolated the ones we thought were interesting. We also wanted to have a good balance of socioeconomic and environmental variables. Below are the variables we chose along with a new summary table.
...
file_path <- "/Users/ayeshasaeed/Documents/Spring 2024/ENGRD 302W/EDA_3_22.rmd" # Provide the correct file path to your R Markdown file

word_count <- rmdcount(file_path) # Get word count, excluding code chunks

print(word_count) # Print word count
```
